...
class TestDateFiltersIntegration:
    ...
    @pytest.mark.asyncio
    async def test_search_reservations_with_date_filters_params_transformation(
        ...
        mock_response = {
            ...
            "_links": {
                "self": {
                    "hre": "https://ihmvacations.trackhs.com/api/v2/pms/reservations/?page=1&size=5&sortColumn=name&sortDirection=asc&status=Confirmed&arrivalStart=2025-01-01T00:00:00Z&arrivalEnd=2025-01-31T23:59:59Z"
                }
            },
        }
        ...
        mock_api_client.client.request.return_value.headers = {
    ...
    async def test_date_normalization_formats(self, mock_mcp, mock_api_client):
        ...
        for input_date, expected_output in test_cases:
            ...
            mock_response = {
                ...
                "page_size": 5,
                "total_items": 0,
                "_links": {"self": {"hre": "test"}},
            }

            mock_api_client.client.request.return_value = AsyncMock()
            mock_api_client.client.request.return_value.is_success = True
            mock_api_client.client.request.return_value.json.return_value = (
                mock_response
            )
            mock_api_client.client.request.return_value.headers = {
                "content-type": "application/json"
            ...
            # Ejecutar búsqueda
            await search_func(arrival_start=input_date, status="Confirmed")

            # Validar normalización
            assert (
                captured_params["arrivalStart"] == expected_output
    ...
    async def test_all_date_parameters_included(self, mock_mcp, mock_api_client):
        ...
        for param_name, expected_value in expected_date_params.items():
            ...
            assert (
                captured_params[param_name] == expected_value
            ), f"Wrong value for {param_name}: {captured_params[param_name]} != {expected_value}"
