...
class TestGetFolioCritical:
    ...
    @pytest.mark.asyncio
    async def test_get_folio_by_id_success(
        self, setup_tool, mock_api_client, sample_folio_data
    ):
        """Test: Obtener folio por ID exitosa"""
        # Arrange
        folio_id = 12345
        mock_api_client.get.return_value = sample_folio_data

        # Act
        result = await setup_tool(folio_id=folio_id)

        # Assert
        assert result == sample_folio_data
        assert result["id"] == folio_id
        mock_api_client.get.assert_called_once_with(f"/pms/folios/{folio_id}")

    @pytest.mark.asyncio
    async def test_get_folio_invalid_id(self, setup_tool):
        """Test: ID de folio inválido"""
        # Act & Assert
        with pytest.raises(Exception):  # Pydantic validation error
            await setup_tool(folio_id="invalid-id")

    @pytest.mark.asyncio
    async def test_get_folio_not_found(self, setup_tool, mock_api_client):
        ...
        with pytest.raises(HTTPStatusError) as exc_info:
            await setup_tool(folio_id=999999)

        assert exc_info.value.response.status_code == 404

    @pytest.mark.asyncio
    async def test_get_folio_api_error(self, setup_tool, mock_api_client):
        ...
        # Act & Assert
        with pytest.raises(HTTPStatusError) as exc_info:
            await setup_tool(folio_id=12345)

        assert exc_info.value.response.status_code == 500

    @pytest.mark.asyncio
    async def test_get_folio_unauthorized(self, setup_tool, mock_api_client):
