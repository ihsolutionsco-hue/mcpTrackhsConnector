...
class TestServerE2E:
    ...
    def test_server_initialization(self, mock_config):
        ...
        with patch('src.trackhs_mcp.server.TrackHSConfig') as mock_config_class, \
    ...
    def test_server_import_structure(self):
        ...
        try:
            from src.trackhs_mcp.server import (
                os, sys, Path, load_dotenv, FastMCP,
                TrackHSApiClient, TrackHSConfig, register_all_components, main
            )
    ...
    def test_server_path_manipulation(self):
        ...
        with patch('src.trackhs_mcp.server.sys') as mock_sys, \
             patch('src.trackhs_mcp.server.Path') as mock_path:
            
            mock_path.return_value.parent.parent = Mock()
            mock_path.return_value.parent.parent.__str__ = Mock(return_value="/test/path")
    ...
    async def test_server_component_registration_flow(self, mock_config):
        ...
        with patch('src.trackhs_mcp.server.TrackHSConfig') as mock_config_class, \
            ...
            assert any("search_reservations" in str(call) for call in tool_calls), "search_reservations not registered"
            
            # Verificar recursos específicos
            assert any("reservations" in str(call) for call in resource_calls), "reservations resource not registered"
            assert any("units" in str(call) for call in resource_calls), "units resource not registered"
            assert any("status" in str(call) for call in resource_calls), "status resource not registered"
            
            # Verificar prompts específicos
            assert any("check-today" in str(call) for call in prompt_calls), "check-today prompt not registered"
            assert any("unit-availability" in str(call) for call in prompt_calls), "unit-availability prompt not registered"
