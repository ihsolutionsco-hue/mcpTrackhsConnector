"""
Tests prácticos para create_maintenance_work_order.
Se enfoca en funcionalidad real que se puede probar sin validaciones de Pydantic.
"""
...
class TestMaintenanceWorkOrderFunctionality:
    """Tests de funcionalidad básica"""

    def test_function_exists_and_callable(self):
        """Verifica que la función existe y es llamable"""
        assert create_maintenance_work_order is not None
        assert callable(create_maintenance_work_order.fn)

    def test_function_parameters(self):
        """Verifica que la función tiene los parámetros correctos"""
        import inspect
...
class TestMaintenanceWorkOrderDataPreparation:
    ...
    def test_data_structure_with_minimal_parameters(self):
        ...
        with patch("src.trackhs_mcp.server.api_client") as mock_client:
            ...
            # dateReceived debe estar presente
    ...
    def test_optional_parameters_handling(self):
        ...
        with patch("src.trackhs_mcp.server.api_client") as mock_client:
            ...
            assert data["estimatedCost"] == 100.0
            assert "estimatedTime" not in data

            # Test con estimated_time pero sin estimated_cost
            result = create_maintenance_work_order.fn(
                unit_id=123, summary="Test", description="Test", estimated_time=60
            )

            call_args = mock_client.post.call_args
            data = call_args[0][1]

            assert data["estimatedTime"] == 60
            assert "estimatedCost" not in data
...
class TestMaintenanceWorkOrderIntegration:
    ...
    def test_complete_workflow_simulation(self):
        ...
        with patch("src.trackhs_mcp.server.api_client") as mock_client:
            ...
            assert result["status"] == "pending"
            assert result["unitId"] == 456
            assert result["priority"] == 5
            assert result["summary"] == "Aire acondicionado no funciona"
            assert (
                result["description"] == "AC no enfría, revisar termostato y compresor"
            )
            assert result["estimatedCost"] == 150.0
            assert result["estimatedTime"] == 120
            assert result["dateReceived"] == "2024-01-15"
...
if __name__ == "__main__":
    print("🧪 Ejecutando tests prácticos de create_maintenance_work_order...")

    # Ejecutar tests específicos
    test_classes = [
        TestMaintenanceWorkOrderFunctionality,
