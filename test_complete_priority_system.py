    ...
    for text_priority, expected_numeric in PRIORITY_MAPPING.items():
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(
                    f"❌ {text_priority} → {expected_numeric} (Error: {response.status_code})"
                )
...
def test_customer_scenarios():
    ...
    customer_scenarios = [
        ...
        {"issue": "Limpieza programada", "priority": "low", "expected_numeric": 1},
        {"issue": "Cambio de bombilla", "priority": "trivial", "expected_numeric": 1},
    ]
    ...
    for scenario in customer_scenarios:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                test_stats["work_orders_created"] += 1
                print(
                    f"✅ {scenario['priority'].upper()}: {scenario['issue']} (ID: {work_order_id})"
                )
            else:
                print(
                    f"❌ {scenario['priority'].upper()}: {scenario['issue']} (Error: {response.status_code})"
                )

        except Exception as e:
            print(
                f"❌ {scenario['priority'].upper()}: {scenario['issue']} (Excepción: {e})"
            )
...
def test_error_handling():
    ...
    invalid_payloads = [
        ...
        {"payload": {"priority": "urgent"}, "expected_error": "Priority is invalid"},
        {"payload": {"priority": "normal"}, "expected_error": "Priority is invalid"},
    ]
        ...
        try:
            ...
            if response.status_code == 422:
                ...
                if "validation_messages" in error_data:
                    print(
                        f"✅ Error manejado correctamente: {error_data['validation_messages']}"
                    )
                else:
                    print(f"✅ Error 422 recibido como esperado")
            else:
                print(
                    f"❌ Debería haber fallado con 422, pero recibió: {response.status_code}"
                )
...
def generate_final_report():
    ...
    success_rate = (
        (test_stats["passed_tests"] / test_stats["total_tests"] * 100)
    ...
    for priority, stats in test_stats["priority_tests"].items():
        if stats["tested"] > 0:
            ...
            print(
                f"   • {priority.upper()}: {stats['passed']}/{stats['tested']} ({pass_rate:.1f}%)"
            )
