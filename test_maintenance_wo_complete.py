            ...
            if response.is_success:
                ...
                try:
                    ...
                    if 'validation_messages' in error_json:
                        error_detail += f" | Validation: {error_json['validation_messages']}"
                except:
                    pass
...
async def test_customer_service_calls():
    ...
    print("\n🔧 CASO 1: Aire acondicionado no funciona")
    print("Llamada: 'Buenos días, tengo un huésped en la Villa 5 que dice que el aire acondicionado no enciende. Hace mucho calor y están muy molestos. ¿Pueden ayudarme?'")
    ...
    if result_ac["success"]:
        ...
        print("💬 INFORMACIÓN PARA EL CLIENTE: 'Señora González, he creado una orden de trabajo prioritaria (Prioridad 5/5). El técnico debería revisar el aire acondicionado en aproximadamente 2 horas. El costo estimado es de $150. Le mantendremos informada del progreso.'")
        print("📋 SEGUIMIENTO: Notificar al equipo técnico inmediatamente, contactar al huésped cada 30 minutos con actualizaciones, considerar cambio de habitación si la reparación toma más de 3 horas")
    else:
        print(f"❌ RESPUESTA DEL SISTEMA: Error - {result_ac['error']}")
        print("💬 INFORMACIÓN PARA EL CLIENTE: 'Señora González, hay un problema técnico con el sistema de órdenes de trabajo. Voy a contactar directamente al plomero de emergencia mientras resuelvo el problema del sistema.'")
    
    # CASO 2: Fuga de agua en villa 10
    print("\n💧 CASO 2: Fuga de agua en villa 10")
    print("Llamada: '¡Emergencia! La Villa 10 tiene una fuga de agua en el baño. El agua está saliendo por debajo de la puerta. Hay huéspedes dentro y están alarmados.'")
    ...
    if result_water["success"]:
        ...
        print("💬 INFORMACIÓN PARA EL CLIENTE: 'He activado el protocolo de emergencia. El técnico está en camino (tiempo estimado: 15 minutos). Por favor, cierre la llave de paso del agua en el baño si es posible. Voy a coordinar con los huéspedes para reubicarlos temporalmente si es necesario.'")
        print("📋 SEGUIMIENTO: Llamar al plomero de emergencia, cerrar llave de paso principal, ofrecer cambio de villa a los huéspedes, documentar daños con fotos")
    else:
        print(f"❌ RESPUESTA DEL SISTEMA: Error - {result_water['error']}")
        print("💬 INFORMACIÓN PARA EL CLIENTE: 'He intentado crear la orden de emergencia pero hay un problema técnico con el sistema. Voy a contactar directamente al plomero de emergencia mientras resuelvo el problema del sistema.'")
    
    # CASO 3: Problemas de WiFi
    print("\n📡 CASO 3: Problemas de WiFi")
    print("Llamada: 'Tengo varios huéspedes quejándose del WiFi. Algunos dicen que no conecta y otros que es muy lento. ¿Es problema del router o del proveedor?'")
    ...
    if result_wifi["success"]:
        ...
        print("💬 INFORMACIÓN PARA EL CLIENTE: 'He creado una orden de trabajo para el equipo técnico. Van a revisar el router y la conexión con el proveedor. Tiempo estimado de resolución: 1.5 horas. Si es problema del proveedor de internet, puede tomar más tiempo. Mantendré informados a los huéspedes.'")
        print("📋 SEGUIMIENTO: Reiniciar router principal como solución temporal, verificar número de dispositivos conectados, contactar al ISP si problema persiste")
    else:
        print(f"❌ RESPUESTA DEL SISTEMA: Error - {result_wifi['error']}")
        print("💬 INFORMACIÓN PARA EL CLIENTE: 'He intentado crear la orden para el problema de WiFi pero el sistema tiene problemas técnicos. Voy a reiniciar el router como solución temporal y contactar al proveedor de internet directamente.'")
    
    # CASO 4: Limpieza profunda post-fiesta
    print("\n🧹 CASO 4: Limpieza profunda post-fiesta")
    print("Llamada: 'La Villa Paraíso necesita limpieza profunda urgente. Los huéspedes tuvieron una fiesta y dejaron todo muy sucio. El próximo check-in es mañana a las 3 PM.'")
    ...
    if result_cleaning["success"]:
        ...
        print("💬 INFORMACIÓN PARA EL CLIENTE: 'He programado el equipo de limpieza profunda para hoy a las 2 PM. Van a trabajar 6 horas aproximadamente. El costo es de $350. La villa estará lista para el check-in de mañana. Voy a hacer una inspección final mañana a las 11 AM para confirmar que todo esté perfecto.'")
        print("📋 SEGUIMIENTO: Confirmar disponibilidad del equipo de limpieza, tomar fotos del estado actual, coordinar con housekeeping para suministros extra, inspección de calidad antes del check-in")
    else:
        print(f"❌ RESPUESTA DEL SISTEMA: Error - {result_cleaning['error']}")
        print("💬 INFORMACIÓN PARA EL CLIENTE: 'He intentado programar la limpieza profunda pero hay un problema técnico con el sistema. Voy a contactar directamente al equipo de limpieza mientras resuelvo el problema del sistema.'")
    
    # CASO 5: Mejoras en la villa
    print("\n🔨 CASO 5: Mejoras en la villa")
    print("Llamada: 'Soy propietario de la Villa Sunset. Quiero instalar un jacuzzi nuevo y mejorar la terraza. No tengo reservas en febrero. ¿Pueden coordinar las mejoras?'")
    ...
    if result_improvements["success"]:
        ...
        print("💬 INFORMACIÓN PARA EL CLIENTE: 'Excelente, Doña Ana. He creado el proyecto de mejoras para iniciar el 1 de febrero. Duración estimada: 2 semanas. Costo total: $8,500. He bloqueado la villa durante febrero completo para asegurar tiempo suficiente. Le enviaré actualizaciones semanales con fotos del progreso.'")
        print("📋 SEGUIMIENTO: Confirmar disponibilidad del contratista, enviar recordatorio al propietario 24 horas antes, bloquear villa en el calendario hasta completar trabajo, inspeccionar trabajo antes de desbloquear")
    else:
        print(f"❌ RESPUESTA DEL SISTEMA: Error - {result_improvements['error']}")
        print("💬 INFORMACIÓN PARA EL CLIENTE: 'He intentado crear el proyecto de mejoras pero hay un problema técnico con el sistema. Voy a contactar directamente al contratista mientras resuelvo el problema del sistema.'")
...
async def test_error_cases():
    ...
    if not result_no_unit["success"]:
        ...
        print(f"❌ INESPERADO - Debería haber fallado pero funcionó: {result_no_unit['work_order_id']}")
    ...
    result_invalid_priority = await call_api(payload_invalid_priority, "Prioridad inválida")
    if not result_invalid_priority["success"]:
        print(f"✅ ERROR ESPERADO - {result_invalid_priority['error']}")
    else:
        print(f"❌ INESPERADO - Debería haber fallado pero funcionó: {result_invalid_priority['work_order_id']}")
    ...
    if not result_invalid_date["success"]:
        ...
        print(f"❌ INESPERADO - Debería haber fallado pero funcionó: {result_invalid_date['work_order_id']}")
