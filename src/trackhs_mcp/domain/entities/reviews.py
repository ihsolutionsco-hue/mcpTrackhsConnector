...
class Review(BaseModel):
    ...
    private_review: Optional[str] = Field(
        default=None, description="Contenido privado de la reseña"
    )
    ...
    status: Literal["published", "pending", "rejected"] = Field(
    ...
    response_date: Optional[str] = Field(
        default=None, description="Fecha de respuesta (ISO 8601)"
    )
...
class ReviewsResponse(BaseModel):
    ...
    pagination: dict[str, int | bool] = Field(
...
class ReviewFilters(BaseModel):
    ...
    status: Optional[Literal["published", "pending", "rejected"]] = Field(
        default=None, description="Estado de la reseña"
    )
    ...
    rating: Optional[int] = Field(
        default=None, ge=1, le=5, description="Calificación mínima"
    )
