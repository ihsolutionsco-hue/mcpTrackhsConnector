...
class ReservationNote(BaseModel):
    ...
    priority: Optional[Literal["low", "medium", "high"]] = Field(
        default=None, description="Prioridad"
    )
...
class GetReservationNotesParams(PaginationParams, SearchParams):
    ...
    sort_by: Optional[Literal["createdAt", "updatedAt", "author", "priority"]] = Field(
        default="createdAt", description="Campo para ordenar"
    )
    sort_direction: Optional[Literal["asc", "desc"]] = Field(
        default="desc", description="Dirección de ordenamiento"
    )
...
class CreateReservationNoteRequest(BaseModel):
    ...
    note_type: Optional[str] = Field(default=None, description="Tipo de nota")
    priority: Optional[Literal["low", "medium", "high"]] = Field(
        default="medium", description="Prioridad"
    )
    tags: Optional[List[str]] = Field(default=None, description="Tags")
