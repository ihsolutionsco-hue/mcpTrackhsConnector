...
class GetContactsParams(PaginationParams, SearchParams):
    ...
    sort_column: Optional[Literal["id", "name", "email", "cellPhone", "homePhone", "otherPhone", "vip"]] = Field(default="id", description="Columna para ordenar")
...
class ContactFilters(BaseModel):
    ...
    is_blacklist: Optional[bool] = Field(default=None, description="Si está en lista negra")
    country: Optional[str] = Field(default=None, description="País")
    region: Optional[str] = Field(default=None, description="Región")
