...
class TrackHSCompletion:
    ...
    def _filter_suggestions(
        self, suggestions: List[CompletionSuggestion], current_input: str
        ...
        for suggestion in suggestions:
            if (
                current_lower in suggestion.value.lower()
                or current_lower in suggestion.label.lower()
            ):
                filtered.append(suggestion)

        # Ordenar por prioridad y relevancia
        filtered.sort(
            key=lambda x: (-x.priority, x.value.lower().startswith(current_lower))
        )
        return filtered[:10]
    ...
    def get_parameter_suggestions(
        ...
        suggestions.extend(
            [
                ...
                CompletionSuggestion(
                    "bookedEnd", "Booked End", "Fecha de fin de reserva", "date", 5
                ),
                CompletionSuggestion(
                    "arrivalStart",
                    "Arrival Start",
                    "Fecha de inicio de llegada",
                    "date",
                    5,
                ),
                CompletionSuggestion(
                    "arrivalEnd", "Arrival End", "Fecha de fin de llegada", "date", 5
                ),
                CompletionSuggestion(
                    "departureStart",
                    "Departure Start",
                    "Fecha de inicio de salida",
                    "date",
                    5,
                ),
                CompletionSuggestion(
                    "departureEnd", "Departure End", "Fecha de fin de salida", "date", 5
                ),
            ]
        )
        ...
        suggestions.extend(
            [
                CompletionSuggestion(
                    "scroll", "Scroll", "Scroll de Elasticsearch", "special", 4
                ),
                CompletionSuggestion(
                    "inHouseToday",
                    "In House Today",
                    "Filtrar por en casa hoy",
                    "special",
                    4,
                ),
                CompletionSuggestion(
                    "status", "Status", "Estado de la reserva", "filter", 5
                ),
                CompletionSuggestion(
                    "groupId", "Group ID", "ID del grupo", "filter", 4
                ),
                CompletionSuggestion(
                    "checkinOfficeId",
                    "Check-in Office ID",
                    "ID de la oficina de check-in",
                    "filter",
                    4,
                ),
            ]
    ...
    def get_sort_column_suggestions(
        self, context: CompletionContext
    ) -> List[CompletionSuggestion]:
    ...
    async def get_completions(
        ...
        elif context.parameter_name in [
            "bookedStart",
            "bookedEnd",
            "arrivalStart",
            "arrivalEnd",
...
async def get_parameter_completions(
    api_client, parameter_name: str, current_input: str = ""
) -> List[CompletionSuggestion]:
    """Función de conveniencia para obtener completions de parámetros"""
    completion = TrackHSCompletion(api_client)
    context = CompletionContext(
        current_input=current_input, parameter_name=parameter_name
    )
    return await completion.get_completions(context)


async def get_endpoint_completions(
    current_input: str = "",
) -> List[CompletionSuggestion]:
