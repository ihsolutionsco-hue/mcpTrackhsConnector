...
class PaginationUtility:
    ...
    def validate_pagination_params(self, page: int, size: int) -> Dict[str, Any]:
        ...
        if total_requested > self.config.max_total_results:
            raise ValueError(f"Total de resultados solicitados ({total_requested}) excede el límite máximo ({self.config.max_total_results})")
    ...
    def generate_links(self, base_url: str, page_info: PageInfo, query_params: Dict[str, Any]) -> Dict[str, str]:
    ...
    def process_standard_response(self, response: Dict[str, Any], page: int, size: int) -> PaginationResult:
    ...
    async def paginate_async(self, api_client, endpoint: str, params: Dict[str, Any], 
                           mode: PaginationMode = None) -> AsyncIterator[PaginationResult]:
    ...
    async def _scroll_paginate(self, api_client, endpoint: str, params: Dict[str, Any]) -> AsyncIterator[PaginationResult]:
    ...
    async def _standard_paginate(self, api_client, endpoint: str, params: Dict[str, Any]) -> AsyncIterator[PaginationResult]:
        """Paginación estándar"""
        ...
        try:
            ...
            # Páginas siguientes
            current_page = page
            while result.page_info.has_next and current_page < self.config.max_total_results // size:
                current_page += 1
    ...
    def get_summary(self, results: List[PaginationResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_items_per_page": total_items / total_pages if total_pages > 0 else 0,
            "pagination_mode": results[0].metadata.get("scroll_mode", False) and "scroll" or "standard"
        }

# Funciones de conveniencia
async def paginate_reservations(api_client, params: Dict[str, Any], 
                              config: PaginationConfig = None) -> List[Any]:
...
async def get_paginated_summary(api_client, params: Dict[str, Any], 
                              config: PaginationConfig = None) -> Dict[str, Any]:
