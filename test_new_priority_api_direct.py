    ...
    for text_priority, expected_numeric in test_cases:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(
                    f"📊 Prioridad enviada: {expected_numeric}, Prioridad recibida: {result.get('priority', 'N/A')}"
                )
    ...
    for i, scenario in enumerate(scenarios, 1):
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(f"✅ ÉXITO - Work Order ID: {work_order_id}")
                print(
                    f"📊 Prioridad: {numeric_priority} (esperado: {scenario['expected_numeric']})"
                )
            else:
                print(f"❌ ERROR - Status: {response.status_code}")
                print(f"📄 Response: {response.text}")
    ...
    for invalid_priority in invalid_priorities:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                work_order_id = result.get("id")
                print(
                    f"❌ INESPERADO - Debería haber fallado pero funcionó: {work_order_id}"
                )
            else:
