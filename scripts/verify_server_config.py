...
def check_file_content(file_path: str, required_content: List[str], description: str) -> bool:
    ...
    try:
        ...
        if missing_content:
            print(f"❌ {description}: {file_path} (falta contenido: {', '.join(missing_content)})")
            return False
        else:
            print(f"✅ {description}: {file_path}")
            return True
...
def check_server_content() -> Dict[str, bool]:
    ...
    content_checks = [
        ("src/trackhs_mcp/__main__.py", ["from .server import mcp"], "Importación del servidor"),
        ("src/trackhs_mcp/server.py", ["FastMCP", "search_units"], "Configuración del servidor"),
        ("fastmcp.json", ["source", "environment"], "Configuración FastMCP"),
        ("requirements.txt", ["fastmcp", "httpx", "pydantic"], "Dependencias requeridas"),
    ]
    
    results = {}
    for file_path, required_content, description in content_checks:
        results[file_path] = check_file_content(file_path, required_content, description)
...
def generate_recommendations(env_results: Dict[str, bool], 
                           ...
                           dep_results: Dict[str, bool]) -> None:
    ...
    # Verificar variables de entorno
    if not env_results.get("TRACKHS_USERNAME") or not env_results.get("TRACKHS_PASSWORD"):
    ...
    if all(env_results.values()) and all(structure_results.values()) and all(content_results.values()) and all(json_results.values()) and all(dep_results.values()):
...
def main():
    ...
    total_checks = len(env_results) + len(structure_results) + len(content_results) + len(json_results) + len(dep_results)
    passed_checks = sum(env_results.values()) + sum(structure_results.values()) + sum(content_results.values()) + sum(json_results.values()) + sum(dep_results.values())
    ...
    generate_recommendations(env_results, structure_results, content_results, json_results, dep_results)
