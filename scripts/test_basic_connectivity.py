...
def main():
    ...
    if successful_urls:
        ...
        for url, result in results.items():
            print(f"   {url}:")
            print(f"      Resolución DNS: {'✅' if result['domain_resolution'] else '❌'}")
            print(f"      Conectividad HTTPS: {'✅' if result['https_connectivity'] else '❌'}")
            print(f"      Conectividad básica: {'✅' if result['basic_connectivity'] else '❌'}")
        ...
        json.dump({
            ...
            "recommendations": {
                ...
                "fastmcp_cloud_config": {
                    "TRACKHS_API_URL": successful_urls[0] if successful_urls else None
                }
            }
