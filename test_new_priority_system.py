...
async def test_new_priority_system():
    ...
    for priority_text, expected_numeric in priority_mapping.items():
        ...
        if result["success"]:
            ...
            print(f"📊 Prioridad enviada: {expected_numeric}, Prioridad recibida: {actual_priority}")
            
            if actual_priority == expected_numeric:
                print(f"✅ Mapeo correcto: {priority_text} → {actual_priority}")
            else:
                print(f"⚠️ Mapeo inesperado: {priority_text} → {actual_priority} (esperado: {expected_numeric})")
        else:
...
async def test_priority_examples():
    ...
    for example in examples:
        ...
        if result["success"]:
            print(f"✅ ÉXITO - Work Order ID: {result['work_order_id']}")
            print(f"📊 Prioridad: {result.get('priority')} (esperado: {example['expected_numeric']})")
        else:
            print(f"❌ ERROR - {result['error']}")
...
async def test_invalid_priorities():
    ...
    for invalid_priority in invalid_priorities:
        ...
        if not result["success"]:
            ...
            print(f"❌ INESPERADO - Debería haber fallado pero funcionó: {result['work_order_id']}")
