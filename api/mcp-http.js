/**
 * Servidor MCP HTTP para Vercel
 * Implementación del protocolo MCP sobre HTTP para Claude
 */
...
async function handleDefault(req, res) {
  res.status(200).json({
    service: 'Track HS MCP Server',
    version: '1.0.0',
    description: 'Servidor MCP para Track HS API',
    endpoints: {
      health: {
        method: 'GET',
        path: '/api/mcp/health',
      ...
      execute: {
        method: 'POST',
        path: '/api/mcp/tools/{name}/execute',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (path === '/api/mcp/health' || path === '/mcp/health' || path === '/api/mcp') {
      return handleHealth(req, res);
    }

    if (path === '/api/mcp/tools' || path === '/mcp/tools') {
      return handleListTools(req, res);
    }

    if (path.startsWith('/api/mcp/tools/') || (path.startsWith('/mcp/tools/') && path.includes('/execute'))) {
