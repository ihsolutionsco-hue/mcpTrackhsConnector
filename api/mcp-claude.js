/**
 * Servidor MCP Real para Claude - Track HS
 * Implementación correcta del protocolo MCP para Claude Web
 */
...
async function handleHealth(req, res) {
  try {
    ...
    res.status(200).json({
      status: 'healthy',
      service: 'Track HS MCP Claude Server',
      version: '1.0.0',
      timestamp: new Date().toISOString(),
      tools: {
        count: tools.length,
...
async function handleDefault(req, res) {
  res.status(200).json({
    service: 'Track HS MCP Claude Server',
    version: '1.0.0',
    description: 'Servidor MCP real para Track HS API - Compatible con Claude',
    endpoints: {
      health: {
        method: 'GET',
        path: '/api/mcp-claude/health',
        description: 'Health check del servicio MCP'
      },
      tools: {
        method: 'GET',
        path: '/api/mcp-claude/tools',
        description: 'Listar herramientas MCP disponibles'
      },
      execute: {
        method: 'POST',
        path: '/api/mcp-claude/tools/{name}/execute',
    ...
    claudeConnector: {
      url: 'https://trackhs-mcp-connector.vercel.app/api/mcp-claude',
      description: 'URL para configurar en Claude MCP Connector'
    },
    timestamp: new Date().toISOString()
  });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (path === '/api/mcp-claude/health' || path === '/mcp-claude/health' || path === '/api/mcp-claude') {
      return handleHealth(req, res);
    }

    if (path === '/api/mcp-claude/tools' || path === '/mcp-claude/tools') {
      return handleListTools(req, res);
    }

    if (path.startsWith('/api/mcp-claude/tools/') || (path.startsWith('/mcp-claude/tools/') && path.includes('/execute'))) {
    ...
    console.error('Error en servidor MCP Claude:', error);
    res.status(500).json({
