/**
 * Servidor MCP Remoto Real para Track HS
 * Implementación del protocolo MCP para Custom Connectors de Claude
 */
...
async function handleHealth(req, res) {
  try {
    ...
    res.status(200).json({
      status: 'healthy',
      service: 'Track HS MCP Remote Server',
      version: '1.0.0',
      timestamp: new Date().toISOString(),
      tools: {
        count: tools.length,
...
async function handleDefault(req, res) {
  res.status(200).json({
    service: 'Track HS MCP Remote Server',
    version: '1.0.0',
    description: 'Servidor MCP remoto para Track HS API - Compatible con Claude Custom Connectors',
    endpoints: {
      health: {
        method: 'GET',
        path: '/api/remote-mcp/health',
        description: 'Health check del servicio MCP remoto'
      },
      tools: {
        method: 'GET',
        path: '/api/remote-mcp/tools',
        description: 'Listar herramientas MCP disponibles'
      },
      execute: {
        method: 'POST',
        path: '/api/remote-mcp/tools/{name}/execute',
    ...
    claudeConnector: {
      ...
      description: 'URL para configurar en Claude Custom Connectors'
    },
    timestamp: new Date().toISOString()
  });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (path === '/api/remote-mcp/health' || path === '/remote-mcp/health' || path === '/api/remote-mcp') {
      return handleHealth(req, res);
    }

    if (path === '/api/remote-mcp/tools' || path === '/remote-mcp/tools') {
      return handleListTools(req, res);
    }

    if (path.startsWith('/api/remote-mcp/tools/') || (path.startsWith('/remote-mcp/tools/') && path.includes('/execute'))) {
    ...
    console.error('Error en servidor MCP remoto:', error);
    res.status(500).json({
