/**
 * Servidor MCP con SSE Real para Track HS
...
async function handleHealth(req, res) {
  try {
    ...
    res.status(200).json({
      status: 'healthy',
      service: 'Track HS MCP SSE Real Server',
      version: '1.0.0',
      timestamp: new Date().toISOString(),
      tools: {
        count: tools.length,
...
async function handleDefault(req, res) {
  res.status(200).json({
    service: 'Track HS MCP SSE Real Server',
    version: '1.0.0',
    description: 'Servidor MCP con SSE real para Track HS API - Compatible con Claude MCP Connector',
    endpoints: {
      health: {
        method: 'GET',
        path: '/api/mcp-sse-real/health',
        description: 'Health check del servicio MCP SSE'
      },
      tools: {
        method: 'GET',
        path: '/api/mcp-sse-real/tools',
        description: 'Listar herramientas MCP disponibles'
      },
      execute: {
        method: 'POST',
        path: '/api/mcp-sse-real/tools/{name}/execute',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (path === '/api/mcp-sse-real/health' || path === '/mcp-sse-real/health' || path === '/api/mcp-sse-real') {
      return handleHealth(req, res);
    }

    if (path === '/api/mcp-sse-real/tools' || path === '/mcp-sse-real/tools') {
      return handleListTools(req, res);
    }

    if (path.startsWith('/api/mcp-sse-real/tools/') || (path.startsWith('/mcp-sse-real/tools/') && path.includes('/execute'))) {
    ...
    console.error('Error en servidor MCP SSE real:', error);
    res.status(500).json({
