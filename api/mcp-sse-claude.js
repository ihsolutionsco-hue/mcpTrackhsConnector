/**
 * Servidor MCP SSE Real para Claude Web - Track HS
 * Implementación correcta del protocolo MCP con Server-Sent Events para Claude
 */
...
async function handleHealth(req, res) {
  try {
    ...
    res.status(200).json({
      status: 'healthy',
      service: 'Track HS MCP SSE Claude Server',
      version: '1.0.0',
      timestamp: new Date().toISOString(),
      tools: {
        count: tools.length,
...
async function handleDefault(req, res) {
  res.status(200).json({
    service: 'Track HS MCP SSE Claude Server',
    version: '1.0.0',
    description: 'Servidor MCP con SSE real para Track HS API - Compatible con Claude',
    endpoints: {
      health: {
        method: 'GET',
        path: '/api/mcp-sse-claude/health',
        description: 'Health check del servicio MCP SSE'
      },
      tools: {
        method: 'GET',
        path: '/api/mcp-sse-claude/tools',
        description: 'Listar herramientas MCP disponibles'
      },
      execute: {
        method: 'POST',
        path: '/api/mcp-sse-claude/tools/{name}/execute',
    ...
    claudeConnector: {
      ...
      description: 'URL para configurar en Claude MCP Connector (con SSE real)'
    },
    timestamp: new Date().toISOString()
  });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (path === '/api/mcp-sse-claude/health' || path === '/mcp-sse-claude/health' || path === '/api/mcp-sse-claude') {
      return handleHealth(req, res);
    }

    if (path === '/api/mcp-sse-claude/tools' || path === '/mcp-sse-claude/tools') {
      return handleListTools(req, res);
    }

    if (path.startsWith('/api/mcp-sse-claude/tools/') || (path.startsWith('/mcp-sse-claude/tools/') && path.includes('/execute'))) {
    ...
    console.error('Error en servidor MCP SSE Claude:', error);
    res.status(500).json({
