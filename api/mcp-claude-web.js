/**
 * Servidor MCP específico para Claude Web
 * Implementación que maneja el protocolo MCP correctamente para Claude Web
 */
...
// Definir herramientas MCP para Claude Web
const MCP_TOOLS = [
  {
    name: "get_contacts",
...
// Manejar request JSON-RPC 2.0 para Claude Web
      ...
      case 'initialize':
        return {
          ...
          result: {
            ...
            serverInfo: {
              name: 'trackhs-mcp-claude-web-server',
              version: '1.0.0'
            }
          }
        };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.url === '/health' || req.url.endsWith('/health')) {
      res.status(200).json({
        status: 'healthy',
        service: 'Track HS MCP Claude Web Server',
        version: '1.0.0',
    ...
    // MCP JSON-RPC 2.0 endpoint para Claude Web
    if (req.method === 'POST') {
    ...
    // Default response
    res.status(200).json({
      message: 'Track HS MCP Claude Web Server',
      version: '1.0.0',
      endpoints: {
        health: '/health',
        tools: '/tools',
    ...
    console.error('Error en servidor MCP Claude Web:', error);
    res.status(500).json({
